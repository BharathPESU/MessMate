...
const QRScanner = ({ onScan, onError }) => {
  ...
  // Always render the qr-reader element so Html5Qrcode can find it when startScanner runs.
  // Show overlays for loading/error states on top of the scanner area.
  return (
    <div className="qr-scanner-container relative">
      <div
        ...
        style={{ width: '100%', maxWidth: '500px', margin: '0 auto', minHeight: 300 }}
      />

      {/* Overlay: error */}
      {cameraError && (
        <div className="absolute inset-0 flex items-center justify-center p-8">
          <div className="glass-dark neon-border text-center p-8 rounded-xl border-2 border-red-500/30 shadow-cyber max-w-xl">
            <div className="mb-4">
            ...
            <div className="space-y-3">
              <button onClick={handleRetry} className="cyber-btn">Retry Camera</button>
              <p className="text-xs text-neon-cyan/60 mt-4 font-rajdhani">💡 For HTTP camera access: Use <span className="font-mono text-neon-green px-1.5 py-0.5 bg-cyber-black/50 rounded">http://localhost:5173</span> instead of IP address</p>
              <p className="text-xs text-neon-cyan/60 font-rajdhani">🔒 Click lock icon in address bar → Camera → Allow</p>
            </div>
          </div>
        </div>
      )}

      {/* Overlay: starting loader */}
      {!isScanning && !cameraError && (
        <div className="absolute inset-0 flex items-center justify-center p-10">
          <div className="glass-dark text-center p-10 rounded-xl border border-neon-green/20 shadow-cyber max-w-lg">
            <div className="mb-4">
      ...
      {/* Helper caption when camera active */}
      {isScanning && !cameraError && (
